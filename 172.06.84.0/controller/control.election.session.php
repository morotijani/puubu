...
if (isset($_POST['election-session'])) {
	...
	$query = "SELECT * FROM election WHERE election_id = ?";
	$statement = $conn->prepare($query);
	$statement->execute([$startElecID]);
	$result_count = $statement->rowCount();

	if ($result_count > 0) {
		// $queryStop = "UPDATE election SET session = '2' WHERE election_id = '".$startElecID."'";
		...
		// }
	//} else {
		$queryStart = "UPDATE election SET session = ?, stop_timer = ? WHERE election_id = ?";
